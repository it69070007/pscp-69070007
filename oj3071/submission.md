...
```text
โจทย์ให้รับจำนวนนับ A B d และ r แล้วแสดงจำนวนนับในช่วง [A, B] ที่หารด้วยจำนวนนับ d แล้วเหลือเศษ r ว่ามีกี่จำนวน
Input:
บรรทัดที่ 1 รับค่าจำนวนนับ A แทนเลขในช่วง [A, B]
บรรทัดที่ 2 รับค่าจำนวนนับ B แทนเลขในช่วง [A, B]
บรรทัดที่ 3 รับค่าจำนวนนับ d แทนตัวหาร
บรรทัดที่ 4 รับค่าจำนวนนับ r แทนเศษที่ต้องการ
Output:
จำนวนเลขที่เป็นจำนวนเต็มในช่วง [A, B] ที่หารด้วยจำนวนนับ d แล้วเหลือเศษ r
Constraints: Input ทุกตัวเป็นจำนวนนับ และ ช่วง [A, B] เป็นช่วงปิดจึงต้องพิจารณา A กับ B ด้วย
```
...
```text
Step 1: รับค่าจำนวนนับ A
Step 2: รับค่าจำนวนนับ B
Step 3: รับค่าจำนวนนับ d
Step 4: รับค่าจำนวนนับ r
Step 5: สร้างตัวแปร count ไว้นับจำนวนเต็มที่หารด้วยจำนวนนับ d แล้วเหลือเศษ r โดยกำหนดให้มีค่า = 0
Step 5: สร้างตัวแปร i ให้มีค่า = A
Step 6: สร้าง while loop โดยจะทำงานจนกว่า i จะมากกว่า B
Step 7: ภายใน loop ตรวจสอบว่า i % d มีค่าเท่ากับ r หรือไม่
Step 8: ถ้าใช่ count จะมีค่าเพิ่มขึ้นจากเดิมอีก 1
Step 9: แสดงค่าของตัวแปร count ซึ่งคือ จำนวนเต็มที่หารด้วยจำนวนนับ d แล้วเหลือเศษ r ที่นับได้ในช่วง [A, B]
```
...
```text
คล้ายกันกับแผนแรก แต่ปรับเปลี่ยนให้ทำงานได้ไวขึ้นโดย ให้ i มีค่าเพิ่มขึ้นจากเดิมอีก d ในกรณีที่ i % d มีค่าเท่ากับ r
```
