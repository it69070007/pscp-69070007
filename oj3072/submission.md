...
```text
โจทย์ให้รับข้อความ แล้วแสดงผลจำนวน a e i o u ที่นับได้ในข้อความ
Input:
รับข้อความที่ให้มา
Output:
จำนวนสระที่นับได้ โดยแยกเป็น a e i o u อยู่คนละบรรทัด
Constraints:
output ที่แสดงออกมาจะแสดงแค่สระที่ข้อความนั้นมี จะไม่แสดงว่านับสระได้ 0
```
...
```text
Step 1: รับข้อความ
Step 2: แปลงข้อความเป็นตัวพิมพ์เล็กทั้งหมด
Step 3: สร้างตัวแปร vowel ที่เป็น list มีสมาชิกเป็น "a", "e", "i", "o", "u"
Step 4: สร้าง vowel_count ที่เป็น list ว่าง เพื่อเก็บจำนวนสระที่นับได้
Step 5: นับจำนวน "a" ที่มีอยู่ในข้อความแล้วเพิ่มลง vowel_count
Step 6: นับจำนวน "e" ที่มีอยู่ในข้อความแล้วเพิ่มลง vowel_count
Step 7: นับจำนวน "i" ที่มีอยู่ในข้อความแล้วเพิ่มลง vowel_count
Step 8: นับจำนวน "o" ที่มีอยู่ในข้อความแล้วเพิ่มลง vowel_count
Step 9: นับจำนวน "u" ที่มีอยู่ในข้อความแล้วเพิ่มลง vowel_count
Step 10: สร้าง for loop ที่จะวนจำนวน 5 รอบ
Step 11: ภายใน loop จะตรวจสอบว่า vowel_count index ที่ตรงกับรอบที่ loop (i) มีค่าไม่เท่ากับ 0 ใช่หรือไม่
Step 12: ถ้าใช่ จะแสดงผลออกมาเป็น vowel[i] : vowel_count[i]
```
...
```text
เหมือนกันกับแผนแรก คือนับจำนวนสระแต่ละตัวแล้วเพิ่มลงใน list จากนั้นจึงนำมาตรวจสอบว่ามีสระตัวไหนในข้อความบ้าง ถ้ามีสระตัวนั้นจะแสดงผลเป็น สระตัวนั้น : จำนวนของสระตัวนั้นที่นับได้
```
...
```text
เป็นกรณีที่ข้อความมีสระครบทุกตัว และมีตัวพิมพ์ใหญ่ผสม
```
...
```text
เป็นกรณีที่ข้อความมีสระไม่ครบทุกตัว
```
...
```text
เป็นกรณีที่ข้อความไม่มีสระอยู่เลย
```
