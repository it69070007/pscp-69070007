...
```text
โจทย์ให้รับหมายเลขห้องที่อยู่ แล้วให้แสดงผลจำนวนกำแพงที่น้อยที่สุดที่ต้องพังเพื่อไปถึงห้องหมายเลข 1
Input:
หมายเลขห้องที่อยู่เป็นจำนวนเต็ม
Output:
จำนวนกำแพงน้อยที่สุดที่จะต้องพัง เพื่อเดินทางจากห้องที่รับมา ไปจนถึงห้องหมายเลข 1 เป็นจำนวนเต็ม
Constraints:
ปราสาทเป็นรูปแบบปิรามิด ทางเดียวที่ทำได้คือพังกำแพงกั้นห้องไป แต่ห้ามพังออกไปด้านนอกตัวปราสาท
```
...
```text
Step 1: รับค่าหมายเลขห้องที่อยู่
Step 2: หาชั้นของปราสาทที่หมายเลขนั้นอยู่ โดยหารากที่ 2 ของเลขที่รับมา จากนั้นปัดเศษขึ้น (เลขที่มากที่สุดของแต่ละชั้นคือเลขชั้นยกกำลัง 2)
Step 3: หาจำนวนกำแพงที่ต้องพัง โดยจะแบ่งส่วนที่ต้องพังเป็น 2 ส่วนหลักๆ คือ 1.กำแพงระหว่างชั้น 2.กำแพงภายในชั้น
Step 4: หากำแพงระหว่างชั้นโดยนำ ชั้นของปราสาทที่หมายเลขนั้นอยู่ - 1
Step 5: หากำแพงภายในชั้นโดยนำ ชั้นของปราสาทที่หมายเลขนั้นอยู่ - 2
Step 6: นำทั้ง 2 ส่วนมารวมกัน จะได้จำนวนกำแพงที่ต้องพังสำหรับกรณีที่ห้องนั้นเป็นรูปสามเหลี่ยมคว่ำ
Step 7: ตรวจสอบว่าห้องที่อยู่เป็นสามเหลี่ยมหงายหรือไม่ โดยถ้าชั้นและห้องเป็นเลขคี่หรือเลขคู่เหมือนกัน ห้องนั้นจะเป็นสามเหลี่ยมหงาย
Step 8: ถ้าใช่ จำนวนกำแพงที่ต้องพัง = จำนวนกำแพงที่ต้องพังสำหรับห้องรูปสามเหลี่ยมคว่ำ + 1
Step 9: แสดงผลจำนวนกำแพงที่จะต้องพัง
```
...
```text
เหมือนกันกับแผนแรก คือหาชั้นที่หมายเลขนั้นอยู่ก่อนแล้วจึงหากำแพงที่จะต้องพัง
```
...
```text
เป็นกรณีที่หมายเลขอยู่ในรูปสามเหลี่ยมหงาย
```
...
```text
เป็นกรณีที่หมายเลขอยู่ในรูปสามเหลี่ยมคว่ำ
```
