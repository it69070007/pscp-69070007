...
```text
โจทย์ให้รับคะแนนโหวตรวมของทั้ง 3 คน และคะแนนโหวตที่สูงที่สุด แล้วตรวจสอบว่าจะมีโอกาสที่คะแนนสูงสุดและต่ำสุดต่างกันเกิน 2 คะแนนหรือไม่ ถ้ามีจะแสดง "Surprising" ถ้าไม่มีจะแสดง "Not surprising"
Input:
บรรทัดที่ 1 รับค่าคะแนนโหวตรวมของทั้ง 3 คน
บรรทัดที่ 2 รับค่าคะแนนโหวตสูงสุดของทั้ง 3 คนนี้
Output:
แสดงผลออกมาเป็นข้อความ "Surprising" หรือ "Not surprising"
Constraints:
คะแนนโหวตของ 1 คน เป็นจำนวนจริงที่อยู่ในช่วงตั้งแต่ 0 ถึง 10 คะแนน
```
...
```text
Step 1: รับค่าคะแนนโหวตรวมของทั้ง 3 คน และค่าคะแนนโหวตสูงสุดของทั้ง 3 คนนี้
Step 2: นำค่าคะแนนโหวตรวม - คะแนนโหวตสูงสุด จะได้เป็นคะแนนรวมของอีก 2 คนที่เหลือ
Step 3: สร้างตัวแปรให้มีค่าเท่ากับคะแนนสูงที่สุด
Step 4: นำตัวแปรนั้นมาเข้าลูปลดลงทีละ 1 ไปเรื่อยๆจนกว่าจะถึง 0
Step 5: กำหนดให้คะแนนต่ำสุด = คะแนนรวมของ 2 คนที่เหลือ - ตัวแปรนั้น
Step 6: ตรวจสอบว่าคะแนนต่ำสุดมีค่า >= 0 หรือไม่
Step 7: ถ้าใช่ จะหยุดการลูป
Step 8: ถ้าไม่ จะทำการลดค่าตัวแปรนั้นลงทีละ 1 ไปเรื่อยๆจนกว่าจะเข้าเงื่อนไขข้างต้น
Step 9: ตรวจสอบว่าคะแนนสูงสุด - คะแนนต่ำสุด มีค่า > 2 หรือไม่
Step 10: ถ้าใช่ ให้แสดงผลเป็นข้อความว่า "Surprising"
Step 11: ถ้าไม่ ให้แสดงผลเป็นข้อความว่า "Not surprising"
```
...
```text
เหมือนกันกับแผนแรก เป็นการหาค่าคะแนนต่ำสุดที่เป็นไปได้จากคะแนนที่เหลือของ คะแนนรวม - คะแนนสูงสุด - คะแนนสูงสุดที่จะเป็นไปได้ในลำดับที่ 2
```
...
```text
เป็นกรณีที่เลขเป็นจำนวนเต็ม คะแนนสูงสุดเป็น 10 ได้ 2 ค่า
```
...
```text
เป็นกรณีที่เลขเป็นทศนิยม และคะแนนสูงสุดลำดับ 2 ไม่สามารถเท่าคะแนนสูงสุดได้
```
